...
namespace ShellFiler.GraphicsViewer.Filter {
    ...
    // クラス：シャープフィルター
    //=========================================================================================
    class ComponentSharp : IFilterComponent {
        // フィルターのレベル
        ...
        // 戻り値：なし
        // メ　モ：float parmaList[0]  シャープのレベル(0～1)
        //=========================================================================================
        public void SetParameter(params object[] paramList) {
            m_filterLevel = (float)paramList[0];
        }
        ...
        private void SetPixel(int pos1, int pos2, int pos3, int x) {
            ...
            m_destImage[pos2 + x * next    ] = (byte)(Math.Min(255f, Math.Max(0f, (-srcB21 - srcB12 - srcB32 - srcB23 + srcB22 * 4) * m_filterLevel + srcB22)));
            m_destImage[pos2 + x * next + 1] = (byte)(Math.Min(255f, Math.Max(0f, (-srcG21 - srcG12 - srcG32 - srcG23 + srcG22 * 4) * m_filterLevel + srcG22)));
            m_destImage[pos2 + x * next + 2] = (byte)(Math.Min(255f, Math.Max(0f, (-srcR21 - srcR12 - srcR32 - srcR23 + srcR22 * 4) * m_filterLevel + srcR22)));
            m_destImage[pos2 + x * next + 3] = (byte)(Math.Min(255f, Math.Max(0f, (-srcA21 - srcA12 - srcA32 - srcA23 + srcA22 * 4) * m_filterLevel + srcA22)));
        }
        ...
        private void FilterOptimize(int y) {
            ...
            for (int x = 1; x < m_cxImage - 1; x++) {
                ...
                m_destImage[pos2    ] = (byte)(Math.Min(255f, Math.Max(0f, (-srcB21 - srcB12 - srcB32 - srcB23 + srcB22 * 4) * m_filterLevel + srcB22)));
                m_destImage[pos2 + 1] = (byte)(Math.Min(255f, Math.Max(0f, (-srcG21 - srcG12 - srcG32 - srcG23 + srcG22 * 4) * m_filterLevel + srcG22)));
                m_destImage[pos2 + 2] = (byte)(Math.Min(255f, Math.Max(0f, (-srcR21 - srcR12 - srcR32 - srcR23 + srcR22 * 4) * m_filterLevel + srcR22)));
                m_destImage[pos2 + 3] = (byte)(Math.Min(255f, Math.Max(0f, (-srcA21 - srcA12 - srcA32 - srcA23 + srcA22 * 4) * m_filterLevel + srcA22)));
        ...
        public FilterMetaInfo MetaInfo {
            get {
                return FilterMetaInfo.SharpFilter;
            }
        }
    }
}
