...
namespace ShellFiler.Terminal.TerminalSession {
    ...
    //=========================================================================================
    // クラス：SSHの結果をWindowsにブリッジするストリーム
    //=========================================================================================
    public class TerminalSSHToWindowsStream : Stream {
        // Closeを通知したときtrue
        ...
        // 戻り値：なし
        //=========================================================================================
        public TerminalSSHToWindowsStream(TerminalShellChannel parent, SSHConnection connection) {
            m_notifyClose = false;
            m_parent = parent;
        }
        ...
        //=========================================================================================
        // 機　能：SSHストリームからデータをWindows UIに向けて送る
        // 引　数：[in]buffer    送信内容のバッファ
        // 　　　　[in]offset    バッファ内の開始オフセット
        // 　　　　[in]length    バッファ内の長さ
        // 戻り値：なし
        //=========================================================================================
        public void StreamAddData(byte[] buffer, int offset, int length) {
            m_parent.OnSSHAddData(buffer, offset, length);

            SSHTerminalLogger.WriteLog(m_parent, buffer, offset, length);
        }
        ...
        public override void Write(byte[] buffer, int offset, int count) {
            ...
            StreamAddData(buffer, offset, count);
        }
        //
        // 概要:
