...
namespace ShellFiler.Terminal.TerminalSession.CommandEmulator {
    ...
    //=========================================================================================
    // クラス：トークンの解析を行う:文字列の定義（戻り=なし）
    //=========================================================================================
    public class OSSpecParserSpecify : IOSSpecTokenParser {
        ...
        public bool ParseToken(string line, OSSpecColumnExpect expect, ref int parsePos, out object value) {
            value = null;
            string expectStr = (string)(expect.ExpectParam[0]);
            for (int i = 0; i < expectStr.Length; i++) {
                if (parsePos >= line.Length) {
                    return false;
                }
                if (line[parsePos] != expectStr[i]) {
                    return false;
                }
                parsePos++;
            }
