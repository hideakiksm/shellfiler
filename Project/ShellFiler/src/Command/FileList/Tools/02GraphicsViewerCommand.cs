...
namespace ShellFiler.Command.FileList.Tools {
    ...
    //=========================================================================================
    // クラス：コマンドを実行する
    // カーソル上のファイルをグラフィックビューアで開きます。
    //   書式 　 GraphicsViewer()
    //   引数  　なし
    //   戻り値　なし
    //   対応Ver 0.0.1
    //=========================================================================================
    class GraphicsViewerCommand : FileListActionCommand {
        ...
        public override object Execute() {
            ...
            for (int i = 0; i < uiFileList.Count; i++) {
                if (!uiFileList[i].Attribute.IsDirectory) {
                    ...
                    if (i == uiCursor) {
                        fileListCursor = fileList.Count;
                        fileList.Add(fileName);
                        initialFilePath = FileListViewTarget.FileList.DisplayDirectoryName + fileName;
                    } else if (ImageLoader.IsTargetFile(fileName)) {
                        fileList.Add(fileName);
                    }
                }
            }
            ...
            GraphicsViewerParameter gvParam = GraphicsViewerParameter.CreateForGraphicsViewer(fileList, fileListCursor, srcFileSystem, fileListContext, basePath);
        ...
        public override UIResource UIResource {
            get {
                return UIResource.GraphicsViewerCommand;
            }
        }
    }
}
