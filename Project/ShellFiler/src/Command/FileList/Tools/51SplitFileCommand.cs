...
namespace ShellFiler.Command.FileList.Tools {
    ...
    //=========================================================================================
    // クラス：コマンドを実行する
    // マークファイルを指定サイズまたは指定個数に分割します。
    //   書式 　 SplitFile()
    //   引数  　なし
    //   戻り値　なし
    //   対応Ver 2.1.0
    //=========================================================================================
    class SplitFileCommand : FileListActionCommand {
        ...
        public override object Execute() {
            // 処理開始条件をチェック
            bool canStart = BackgroundTaskCommandUtil.CheckTaskStart(typeof(SplitFileBackgroundTask));
            if (!canStart) {
                return false;
            }
            ...
            if (FileListViewTarget.FileList.MarkedDirectoryCount == 1) {
                InfoBox.Warning(Program.MainWindow, Resources.Msg_SplitDirectoryMarked);
                return null;
            }
            if (FileListViewTarget.FileList.MarkedFileCount == 0) {
                InfoBox.Warning(Program.MainWindow, Resources.Msg_SplitFileCount);
                return null;
            }
            ...
            // 結合条件を入力
            RenameNumberingInfo numberingInfo = RenameNumberingInfo.DefaultSplitUI();
            SplitFileInfo splitInfo = SplitFileInfo.DefaultUI();
            SplitFileDialog dialog = new SplitFileDialog(FileListViewTarget.FileList, FileListViewOpposite.FileList, numberingInfo, splitInfo); 
            DialogResult result = dialog.ShowDialog(Program.MainWindow);
            if (result != DialogResult.OK) {
                return null;
            }
            numberingInfo = dialog.ResultNumberingInfo;
            splitInfo = dialog.ResultSplitFileInfo;
            ...
            IFileProviderSrc srcProvider = new FileProviderSrcMarked(FileListViewTarget.FileList, FileListViewTarget.FileList.MarkFilesExceptFolder, srcFileSystem, srcFileListContext);
            ...
            SplitFileBackgroundTask combineTask = new SplitFileBackgroundTask(srcProvider, destProvider, uiTarget, numberingInfo, splitInfo);
        ...
        public override UIResource UIResource {
            get {
                return UIResource.SplitFileCommand;
            }
        }
    }
}
