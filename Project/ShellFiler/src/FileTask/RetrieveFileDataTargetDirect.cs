...
namespace ShellFiler.FileTask {
    ...
    //=========================================================================================
    // クラス：出力内容を直接指定するデータターゲット
    //=========================================================================================
    class RetrieveFileDataTargetDirect : IRetrieveFileDataTarget {
        // ストリームから受け取った後の文字列（標準出力／標準エラー出力混在）
        private List<string> m_listReceive = new List<string>();

        // プロセスの出力を文字列化する際のエンコーディング
        private Encoding m_encoding = EncodingType.WindowsDefaultEncoding;
        ...
        // 戻り値：なし
        //=========================================================================================
        public RetrieveFileDataTargetDirect() {
        }
        ...
        public void AddData(byte[] buffer, int offset, int length) {
            // 最後は必ず'\n'のはず：-1
            string line = m_encoding.GetString(buffer, offset, length - 1);
            lock (this) {
