...
namespace ShellFiler.FileSystem.SFTP {
    ...
    // クラス：画像情報を取得するプロシージャ
    //=========================================================================================
    class SFTPRetrieveImageProcedure : SFTPProcedure {
        // SSH内部処理のコントローラ
        ...
        // 戻り値：なし
        //=========================================================================================
        public SFTPRetrieveImageProcedure(SSHConnection connection, FileOperationRequestContext context) {
        ...
        public FileOperationStatus Execute(string dirName, long maxSize, out BufferedImage image) {
            ...
            // コマンドを実行
            byte[] buffer;
            string command = m_controler.Connection.ShellCommandDictionary.GetCommandGetFileHead(path, (int)(Math.Min(maxSize + 1, int.MaxValue)));
            try {
