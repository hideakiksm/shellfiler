...
namespace ShellFiler.FileSystem.Transfer {
    ...
    //=========================================================================================
    // クラス：SSHシェル→SSHシェルでのファイル操作API
    //=========================================================================================
    class ShellToShellFileSystem : IFileSystemToFileSystem {
        // 転送先のSSHファイルシステム
        ...
        // 戻り値：なし
        //=========================================================================================
        public ShellToShellFileSystem(ShellFileSystem shellFileSystem) {
        ...
        public FileOperationStatus CopyFile(FileOperationRequestContext context, string srcFilePath, IFile srcFileInfoAttr, string destFilePath, bool overwrite, AttributeSetMode attrMode, FileFilterTransferSetting fileFilter, FileProgressEventHandler progress) {
            ...
            if (sameSession && fileFilter == null) {
                ...
                bool attrCopy = false;
                if (attrMode != null) {
                    attrCopy = attrMode.IsSetAttribute(m_shellFileSystem.FileSystemId);
                }
                status = m_shellFileSystem.RemoteToRemoteFileTransfer(context, TransferModeType.CopyFile, srcFilePath, destFilePath, overwrite, attrCopy, fileFilter, progress);
                return status;
            } else {
                // 違うセッションで処理
                IFileSystemToFileSystem trans = Program.Document.FileSystemFactory.CreateTransferFileSystem(FileSystemID.SSHShell, FileSystemID.SSHShell, true);
        ...
        public FileOperationStatus MoveFileDirectory(FileOperationRequestContext context, string srcFilePath, IFile srcFileInfoAttr, string destFilePath, bool overwrite, AttributeSetMode attrMode, FileProgressEventHandler progress) {
            ...
            if (sameSession) {
                ...
                status = m_shellFileSystem.RemoteToRemoteFileTransfer(context, TransferModeType.MoveFile, srcFilePath, destFilePath, overwrite, attrCopy, null, progress);
                return status;
            } else {
                // 違うセッションで処理
                IFileSystemToFileSystem trans = Program.Document.FileSystemFactory.CreateTransferFileSystem(FileSystemID.SSHShell, FileSystemID.SSHShell, true);
        ...
        public FileOperationStatus CopyDirectoryDirect(FileOperationRequestContext context, string srcPath, string destPath, AttributeSetMode attrMode, FileProgressEventHandler progress) {
            bool attrCopy = attrMode.IsSetAttribute(m_shellFileSystem.FileSystemId);
            return m_shellFileSystem.RemoteCopyDirectoryDirect(context, srcPath, destPath, attrCopy, progress);
        }
        ...
        public FileOperationStatus CreateShortcut(FileOperationRequestContext context, string srcFilePath, string destFilePath, bool overwrite, ShortcutType type) {
            return m_shellFileSystem.CreateShortcut(context, srcFilePath, destFilePath, overwrite, type);
        }
        ...
        public FileOperationStatus CopyFileInfo(FileOperationRequestContext context, bool isDir, string srcFilePath, IFile srcFileInfo, string destFilePath, AttributeSetMode attrMode) {
            FileOperationStatus status;
            bool copyAttr = attrMode.IsSetAttribute(m_shellFileSystem.FileSystemId);
            if (!copyAttr) {
            ...
            // 転送元の属性を取得
            if (srcFileInfo == null) {
                status = m_shellFileSystem.GetFileInfo(context, srcFilePath, true, out srcFileInfo);
                if (!status.Succeeded) {
                    return status;
                }
            }
            ...
            // 属性を設定
            status = m_shellFileSystem.SetFileInfo(context, srcFileInfo, destFilePath, copyAttr, copyAttr);
            return status;
        }
        ...
        public FileOperationStatus CombineFile(FileOperationRequestContext context, List<string> srcFilePath, string destFilePath, ITaskLogger taskLogger) {
            ...
            if (sameSession) {
                // 同じセッションで処理
                status = m_shellFileSystem.CombineFile(context, srcFilePath, destFilePath, taskLogger);
                return status;
            } else {
                // 違うセッションで処理
                IFileSystemToFileSystem trans = Program.Document.FileSystemFactory.CreateTransferFileSystem(FileSystemID.SSHShell, FileSystemID.SSHShell, true);
        ...
        public FileOperationStatus SplitFile(FileOperationRequestContext context, string srcFilePath, string destFolderPath, RenameNumberingInfo numberingInfo, SplitFileInfo splitInfo, ITaskLogger taskLogger) {
            ...
            if (sameSession) {
                // 同じセッションで処理
                status = m_shellFileSystem.SplitFile(context, srcFilePath, destFolderPath, numberingInfo, splitInfo, taskLogger);
                return status;
            } else {
                // 違うセッションで処理
                IFileSystemToFileSystem trans = Program.Document.FileSystemFactory.CreateTransferFileSystem(FileSystemID.SSHShell, FileSystemID.SSHShell, true);
